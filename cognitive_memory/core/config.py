...
class DatabaseConfig:
    ...
    def from_env(cls) -> 'DatabaseConfig':
        ...
        return cls(
            ...
            backup_interval_hours=int(os.getenv('DB_BACKUP_INTERVAL', str(cls.backup_interval_hours))),
...
class CognitiveConfig:
    ...
    def from_env(cls) -> 'CognitiveConfig':
        """Create configuration from environment variables."""
        return cls(
            activation_threshold=float(os.getenv('ACTIVATION_THRESHOLD', str(cls.activation_threshold))),
            bridge_discovery_k=int(os.getenv('BRIDGE_DISCOVERY_K', str(cls.bridge_discovery_k))),
            max_activations=int(os.getenv('MAX_ACTIVATIONS', str(cls.max_activations))),
            consolidation_threshold=int(os.getenv('CONSOLIDATION_THRESHOLD', str(cls.consolidation_threshold))),
            emotional_weight=float(os.getenv('EMOTIONAL_WEIGHT', str(cls.emotional_weight))),
            temporal_weight=float(os.getenv('TEMPORAL_WEIGHT', str(cls.temporal_weight))),
            contextual_weight=float(os.getenv('CONTEXTUAL_WEIGHT', str(cls.contextual_weight))),
...
class SystemConfig:
    ...
    def validate(self) -> bool:
        ...
        if total_weight > 1.0:
            logger.warning(f"Dimension weights sum to {total_weight:.2f}, consider normalizing")
