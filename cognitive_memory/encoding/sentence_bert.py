...
class SentenceBERTProvider(EmbeddingProvider):
    ...
    def encode_batch(self, texts: list[str]) -> torch.Tensor:
        ...
        if not filtered_texts:
            ...
            return torch.zeros((len(texts), self.embedding_dimension), dtype=torch.float32)
        ...
        try:
            ...
            if len(valid_indices) != len(texts):
                full_embeddings = torch.zeros((len(texts), self.embedding_dimension), dtype=torch.float32)
    ...
    def compute_batch_similarity(
        ...
        try:
            ...
            similarities = torch.mm(candidates_norm, query_norm.unsqueeze(-1)).squeeze(-1)
