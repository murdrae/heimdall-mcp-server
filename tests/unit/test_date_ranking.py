...
def test_date_based_ranking():
    ...
    old_memory = CognitiveMemory(
        ...
        hierarchy_level=1,
        cognitive_embedding=base_embedding
        + np.random.randn(384) * 0.1,  # Moderate variation
    ...
    search = SimilaritySearch(memory_storage=storage, cognitive_config=config)

    # Perform search
    query_vector = (
        base_embedding + np.random.randn(384) * 0.1
    )  # Moderately similar to both memories
    ...
    print(
        f"Result 1: {results[0].memory.id} (combined_score: {results[0].combined_score:.6f})"
    )
    print(
        f"Result 2: {results[1].memory.id} (combined_score: {results[1].combined_score:.6f})"
    )
    ...
    print(
        f"Recent memory: similarity={recent_result.similarity_score:.6f}, combined={recent_result.combined_score:.6f}"
    )
    print(
        f"Old memory: similarity={old_result.similarity_score:.6f}, combined={old_result.combined_score:.6f}"
    )
    ...
    print(
        f"Similarity difference: {sim_diff:.6f} (threshold: {config.similarity_closeness_threshold})"
    )
