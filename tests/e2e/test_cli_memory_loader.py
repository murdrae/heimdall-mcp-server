...
"""
        ...
        with sqlite3.connect(str(db_path)) as conn:
            ...
            assert 0 in levels or 1 in levels or 2 in levels, "Should have valid hierarchy levels"
...
"""
    ...
    def test_cli_load_command_invalid_file_format(self, tmp_path):
        ...
        # Execute CLI command
        result = subprocess.run([
            'memory_system', 'load', str(invalid_file)
        ], capture_output=True, text=True)
...
"""
        ...
        lines = result.stdout.split('\n')
        connections_line = next(line for line in lines if "Connections created:" in line)
        ...
        with sqlite3.connect(str(db_path)) as conn:
            ...
            assert len(connections) == connections_count, "Database connections should match CLI output"
...
"""
        ...
        with sqlite3.connect(str(db_path)) as conn:
            ...
            assert actual_count == memories_count, f"CLI reported {memories_count} but DB has {actual_count}"
            
            # Verify hierarchy distribution if shown in output
            hierarchy_lines = [line for line in lines if "L0" in line or "L1" in line or "L2" in line]
            if hierarchy_lines:
                # Validate hierarchy distribution matches database
                db_distribution = dict(conn.execute("""
                    SELECT hierarchy_level, COUNT(*) 
                    FROM memories 
                    GROUP BY hierarchy_level
                """).fetchall())
                
                assert len(db_distribution) > 0, "Should have memories at different hierarchy levels"
    ...
    def test_cli_error_message_quality(self, tmp_path):
        ...
        result = subprocess.run([
            'memory_system', 'load', str(bad_path)
        ], capture_output=True, text=True)
        
        assert result.returncode != 0
        error_output = result.stdout.lower()
        assert any(word in error_output for word in ['not found', 'does not exist', 'file', 'path'])
        ...
        if result.returncode != 0:
            error_output = result.stdout.lower()
            assert any(word in error_output for word in ['unsupported', 'format', 'extension'])

    def test_cli_help_and_usage(self, tmp_path):
        """Test CLI help and usage information."""
        # Test help command
        result = subprocess.run([
            'memory_system', 'load', '--help'
        ], capture_output=True, text=True)
